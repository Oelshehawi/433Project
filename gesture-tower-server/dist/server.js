...
try {
    ...
    if (fs_1.default.existsSync('/etc/letsencrypt/live/your-app/privkey.pem') &&
        fs_1.default.existsSync('/etc/letsencrypt/live/your-app/fullchain.pem')) {
        // Load SSL certificates
        const privateKey = fs_1.default.readFileSync('/etc/letsencrypt/live/your-app/privkey.pem', 'utf8');
        const certificate = fs_1.default.readFileSync('/etc/letsencrypt/live/your-app/fullchain.pem', 'utf8');
        ...
        console.log('Warning: Browser clients over HTTPS will not be able to connect');
    }
}
catch (error) {
...
const sendToRoom = (roomId, event, payload) => {
    clients.forEach((client) => {
        if (client.roomId === roomId && client.readyState === ws_1.default.OPEN) {
...
const parseBeagleBoardCommand = (message) => {
    try {
...
const parseGestureMessage = (message) => {
    try {
...
const handleBeagleBoardCommand = (command, deviceId, params, rinfo) => {
    ...
    switch (command) {
        ...
        case 'JOIN_ROOM':
            ...
            if (RoomID && PlayerName) {
                ...
                sendResponseToBeagleBoard('JOIN_ROOM', 'ERROR', 'Missing RoomID or PlayerName', deviceId, rinfo);
            }
            break;
        case 'LEAVE_ROOM':
            ...
            if (beagleBoards.has(deviceId)) {
                ...
                if (board.roomId) {
                    ...
                    sendResponseToBeagleBoard('LEAVE_ROOM', 'ERROR', 'Not in a room', deviceId, rinfo);
                }
            }
            else {
                sendResponseToBeagleBoard('LEAVE_ROOM', 'ERROR', 'Device not registered', deviceId, rinfo);
            }
            break;
        case 'SET_READY':
            ...
            if (beagleBoards.has(deviceId)) {
                ...
                if (board.roomId) {
                    ...
                    sendResponseToBeagleBoard('SET_READY', 'ERROR', 'Not in a room', deviceId, rinfo);
                }
            }
            else {
                sendResponseToBeagleBoard('SET_READY', 'ERROR', 'Device not registered', deviceId, rinfo);
            }
            break;
        default:
            sendResponseToBeagleBoard(command, 'ERROR', `Unknown command: ${command}`, deviceId, rinfo);
    }
};
// Handle gesture data from Beagle board
const handleGestureData = (deviceId, roomId, gestureData) => {
    try {
        // Parse the gesture data as JSON
        const gestureJson = JSON.parse(gestureData);
        // Ensure a room exists with this ID
        if (!rooms.has(roomId)) {
            console.error(`Room ${roomId} not found for gesture from device ${deviceId}`);
            return;
        }
        ...
        if (!board || !board.playerName) {
            console.error(`Device ${deviceId} not properly registered with a player name`);
            return;
        }
        ...
        console.log(`Processed gesture ${gestureJson.gesture} from device ${deviceId} in room ${roomId}`);
    }
    catch (error) {
...
const sendRoomListToBeagleBoard = (deviceId, rinfo) => {
    const roomList = getRoomList();
    console.log(roomList);
    // Make sure format exactly matches what client expects: RESPONSE:LIST_ROOMS|DeviceID:deviceId|Rooms:json
    const response = `RESPONSE:LIST_ROOMS|DeviceID:${deviceId}|Rooms:${JSON.stringify(roomList)}`;
...
const joinBeagleBoardToRoom = (deviceId, roomId, playerName, rinfo) => {
    // Find the room (case insensitive to be more forgiving with IDs)
    const room = Array.from(rooms.values()).find((r) => r.id.toLowerCase() === roomId.toLowerCase());
    if (!room) {
        sendResponseToBeagleBoard('JOIN_ROOM', 'ERROR', `Room ${roomId} not found`, deviceId, rinfo);
        return;
    }
    // Check if room is full
    if (room.players.length >= room.maxPlayers) {
        sendResponseToBeagleBoard('JOIN_ROOM', 'ERROR', 'Room is full', deviceId, rinfo);
        return;
    }
    ...
    sendResponseToBeagleBoard('JOIN_ROOM', 'SUCCESS', `Joined room ${room.id} successfully`, deviceId, rinfo);
    ...
    console.log(`Beagle board ${deviceId} joined room ${room.id} as player ${playerName}`);
};
// Function to remove a Beagle board from a room
const leaveBeagleBoardFromRoom = (deviceId, roomId, rinfo) => {
    if (!rooms.has(roomId)) {
        sendResponseToBeagleBoard('LEAVE_ROOM', 'ERROR', `Room ${roomId} not found`, deviceId, rinfo);
        return;
    }
    const room = rooms.get(roomId);
    const board = beagleBoards.get(deviceId);
    if (!board || !board.playerName) {
        sendResponseToBeagleBoard('LEAVE_ROOM', 'ERROR', 'Device not properly registered', deviceId, rinfo);
        return;
    }
    // Find the player in the room
    const playerIndex = room.players.findIndex((p) => p.name === board.playerName);
    if (playerIndex === -1) {
        sendResponseToBeagleBoard('LEAVE_ROOM', 'ERROR', `Player ${board.playerName} not found in room`, deviceId, rinfo);
        return;
    }
    // Remove the player
    ...
    sendResponseToBeagleBoard('LEAVE_ROOM', 'SUCCESS', `Left room ${roomId}`, deviceId, rinfo);
};
// Function to send a standardized response to a Beagle board
const sendResponseToBeagleBoard = (command, status, message, deviceId, rinfo) => {
    ...
    udpServer.send(response, rinfo.port, rinfo.address, (err) => {
        if (err) {
            ...
            console.log(`Sent response to device ${deviceId}: ${status} - ${message}`);
        }
    });
};
// Function to extract device ID from a message
const extractDeviceId = (message) => {
...
const processBeagleBoardCommandForWebClients = (command, deviceId, message) => {
...
const handleCreateRoom = (client, payload) => {
    try {
        const { room } = payload;
        // Validate data
        if (!room) {
...
const handleJoinRoom = (client, payload) => {
    try {
...
const handleLeaveRoom = (client, payload) => {
    try {
        ...
        if (playerIndex !== -1) {
            room.players.splice(playerIndex, 1);
            console.log(`Player ${client.playerName} left room ${room.name} (${roomId})`);
            // If room is empty, remove it
            if (room.players.length === 0) {
                rooms.delete(roomId);
...
const handlePlayerReady = (client, payload) => {
    try {
        ...
        console.log(`Player ready event: Room ${effectiveRoomId}, Player ${effectivePlayerId}, Ready: ${isReady}`);
        // Validate data
        ...
        if (!player) {
            console.error(`Player ${effectivePlayerId} not found in room ${effectiveRoomId}`);
            return sendToClient(client, 'error', {
        ...
        console.log(`Player ${player.name} (${effectivePlayerId}) in room ${room.name} (${effectiveRoomId}) is now ${isReady ? 'ready' : 'not ready'}`);
    }
    catch (error) {
...
// Handle game start
const handleGameStart = (client, payload) => {
    try {
        const { roomId } = payload;
        // Validate data
        if (!roomId) {
...
const handleGestureEvent = (client, payload) => {
    try {
        ...
        console.log(`Gesture event: ${gesture} from player ${playerId} (conf: ${confidence})`);
...
const setBeagleBoardReady = (deviceId, roomId, isReady, rinfo) => {
    // Check if room exists
    if (!rooms.has(roomId)) {
        sendResponseToBeagleBoard('SET_READY', 'ERROR', `Room ${roomId} not found`, deviceId, rinfo);
        return;
    }
    const room = rooms.get(roomId);
    const board = beagleBoards.get(deviceId);
    ...
    const player = room.players.find((p) => p.name === board.playerName);
    if (!player) {
        sendResponseToBeagleBoard('SET_READY', 'ERROR', `Player ${board.playerName} not found in room`, deviceId, rinfo);
        return;
    }
    // Update player ready status
    player.isReady = isReady;
    // Send success response to BeagleBoard
    sendResponseToBeagleBoard('SET_READY', 'SUCCESS', `Player is now ${isReady ? 'ready' : 'not ready'}`, deviceId, rinfo);
    // Notify all clients about the room update
    sendToRoom(roomId, 'room_updated', { room });
    // Also broadcast room_updated to ALL clients for better state synchronization
    broadcastToAllClients({
    ...
    console.log(`Beagle board ${deviceId} (${board.playerName}) in room ${roomId} is now ${isReady ? 'ready' : 'not ready'}`);
};
